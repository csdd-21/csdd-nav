...
app.post("/upload", upload.single('file'), function (req, res, next) {
    ...
    let params = new Date().getMinutes()
    // let ext = req.body.file.originalname.split(".")[1]
    // let ext = req.body.file.name.split(".")[1]
    // let targetPath = path.join(process.cwd(), '../src/assets/test/' + params + '.' + ext)

    fs.rename(path.join(process.cwd(), "/" + req.body.file),  path.join(process.cwd(), '../src/assets/test/1.png'), function (err, doc) {
        if (err) {
            console.log('file upload failed ---', err);
            return res.sendResult(null, 400, "上传文件失败");
        }
