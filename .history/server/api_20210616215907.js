...
app.post("/submit", upload.single('file'), function (req, res, next) {
    ...
    let targetPath = '../public/favicon/'+ req.body.favicon.split('\\')[1]
    console.log('888888888888888',path.join(process.cwd(), targetPath));
    
    fs.rename(path.join(process.cwd(), "/" + req.body.favicon), path.join(process.cwd(), targetPath), function (err) {
        if (err) {
            console.log('err 000000000000',err);
            
            // return res.sendResult(null, 400, "提交图片失败");
        }
        res.send({ status: 200, message: "提交图片成功", data: { url: targetPath } })
    })
    // console.log('submit req.file ---',req.file,'submit url ---',targetPath);
});
