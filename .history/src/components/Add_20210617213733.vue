...
export default {
  ...
  methods: {
    ...
    handle_success(res, file) {
      ...
      if (res.status == 200) {
        console.log('999',JSON.parse(JSON.stringify(res.data.url.replace("\\", "\/"))));
        let test = JSON.parse(JSON.stringify(res.data.url.replace("\\", "\/")))
        ...
        this.form.icon = require("../../server/"  +  res.data.url.replace("\\", "\/"));
        this.test = "重新上传";
