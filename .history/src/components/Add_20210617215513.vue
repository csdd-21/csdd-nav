    ...
    <el-dialog :title="info.parent" v-model="showAdd" :before-close="handle_close">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="图标" prop="icon">
          <img :src="form.icon" alt="img" />
          <el-upload list-type="picture-card" action="/api/upload" :file-list="fileList" :before-upload="handle_before" :on-change="handle_change" :on-success="handle_success" v-model="form.icon" list-type="text">
...
export default {
  data() {
    const iconValidate = (rule, value, callback) => {
      ...
      if (value == "/img/default.53e409ef.png") {
  ...
  methods: {
    ...
    handle_success(res, file) {
      ...
      if (res.status == 200) {
        // let url = "../../server/" + res.data.url.replace("\\", "\/")
        // console.log('url ---',url);
        this.form.icon = require("../../server/tmp_uploads/bilibili.png");
        this.test = "重新上传";
  ...
  mounted() {
    Object.keys(this.info).forEach((i) => {
      if (!this.info["icon"]) {
        this.form["icon"] = require("../../public/favicon/default.png");
        this.fileList[0].url =require('../../server/tmp_uploads/bilibili.png')
      }
