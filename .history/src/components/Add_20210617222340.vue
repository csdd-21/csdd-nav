    ...
    <el-dialog :title="info.parent" v-model="showAdd" :before-close="handle_close">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="图标" prop="icon">
          <el-upload action="/api/upload" :limit='1' :file-list="fileList" :before-upload="handle_before" :on-change="handle_change" :on-success="handle_success" v-model="form.icon" list-type="picture-card">
...
export default {
  ...
  mounted() {
    if (this.info["icon"]) {
      this.fileList.push({ name: this.info.title, url: require("../../public" + this.info["icon"]) });
    }
