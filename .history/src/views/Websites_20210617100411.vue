      ...
      <div class="content">
        <div class="part" v-for="(i, index) of datas" :key="index">
          <div class="col-sm-12 col-md-12 col-lg-12 partTitle" :id="i._id">
            <span>{{ i._id }}</span>
            <span class="addBtn" @click="clickAdd({ id: i._id, flag: i.flag, parent: i.parent })">+</span>
            <i class="iconfont icon-edit" @click="click_showEdit"></i>
          </div>
          <div class="col-sm-6 col-md-4 col-lg-3 partContent" v-for="j of i.children" :key="j._id">
            <div class="info">
              <div class="one">
                <span class="imgWrapper"><img :src="j.icon" /></span>
                <a class="url" :href="j.url" target="_blank">
                  {{ j.title }}
                </a>
              </div>

              <div class="two"><i class="iconfont icon-edit" @click="click_edit(j)"></i> <i class="iconfont icon-cuo" @click="click_delete(j)"></i></div>
            </div>
          </div>
        </div>
