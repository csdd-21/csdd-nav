        ...
        <div class="part" v-for="(i, index) of datas" :key="index">
          <div class="col-sm-12 col-md-12 col-lg-12 partTitle" :id="i._id">
            ...
            <div class="action">
              ...
              <i class="iconfont icon-edit2" @click="click_showEdit(index)" v-if="!show.edit.status_1[i.flag]"></i>
  ...
  methods: {
    getDatas(params) {
      ...
      if (!params) {
       ...
       this.axios
          .get("/api/websites",{flag:params.flag,parent:params.parent})
          .then((res) => {
            if (res.data.status == 200) {
              this.datas = res.data.data;
              console.log("----- 默认 -----");
    ...
    close_showAdd(i) {
      this.show.add = i;
      this.getDatas(this.info);
    },

    // 编辑
    click_showEdit(index) {
      this.show.edit.status_1[index] = true;
      this.show.edit.status_2[index] = true;
      this.index = index;
    },
