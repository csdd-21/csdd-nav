        ...
        <div class="part" v-for="(i, index) of datas" :key="index">
          <div class="col-sm-12 col-md-12 col-lg-12 partTitle" :id="i._id">
            ...
            <div class="action">
              ...
              <i class="iconfont icon-edit2" @click="click_showEdit(i.flag)" v-if="!show.editStatus[i.flag]"></i>
              <i class="iconfont icon-bianji" @click="click_closeShowEdit(i.flag)" v-if="show.editStatus[i.flag]"></i>
            </div>
          </div>
          <div class="col-sm-6 col-md-4 col-lg-3 partContent" v-for="j of i.children" :key="j._id">
            <div class="info">
              ...
              <div class="two">
                <i class="iconfont icon-edit1" @click="click_edit(j)" v-show="show.edit[j.flag]"></i>
                <i class="iconfont icon-cuo" @click="click_delete(j)" v-show="show.edit[j.flag]"></i>
              </div>
            </div>
          </div>
        </div>
        <Add v-if="showAdd" :showAdd="showAdd" @close_showAdd="close_showAdd" :info="info" />
      </div>
  ...
  methods: {
    getDatas(url) {
      this.axios
        .get(url)
        .then((res) => {
          console.log("websites axios res ---", res);
          this.datas = JSON.parse(JSON.stringify(res.data));
        })
        .catch(function (err) {
          console.error("websites axios res ---", err);
        })
        .then(function () {});
    },
    click_showAdd(i) {
    ...
    click_showEdit(flag) {
      ...
      console.log("this.show.edit[flag] ", this.show.edit[flag]);
      this.show.editStatus[flag] = true;
      this.show.edit[flag] = true;
    },
    click_closeShowEdit(flag) {
      this.show.editStatus[flag] = false;
